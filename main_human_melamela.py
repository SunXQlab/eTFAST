...
# 全局路径
dir_name ='./results_human_melanoma_0919'
DATA_DIR = "./data/"
PROCESS_DIR = dir_name + "/processed_data/"
MODEL_DIR = dir_name + "/model_params/"
TG_PRED_DIR = dir_name + "/tg_prediction/"
LOSS_DIR = dir_name + "/loss_curves/"
# 创建目录
for dir_path in [MODEL_DIR,PROCESS_DIR, TG_PRED_DIR, LOSS_DIR]:
    os.makedirs(dir_path, exist_ok=True)

def filter_receivers_by_cell_count(adata, min_cells=50, max_cells=500):
    """根据细胞数量划分receivers"""
...
def train_and_evaluate(adata, senders, receivers, is_grouped=False):
    ...
    #保存数据集
    with open(os.path.join(PROCESS_DIR, f"dataset_{save_name}"), "wb") as f:
        pickle.dump(dataset, f)
    with open(os.path.join(PROCESS_DIR, f"TFTG_matrix_{save_name}"), "wb") as f:
        pickle.dump(TFTG_matrix, f)

    #加载数据集
    with open(os.path.join(PROCESS_DIR, f"dataset_{save_name}"), "rb") as f:
        dataset = pickle.load(f)
    with open(os.path.join(PROCESS_DIR, f"TFTG_matrix_{save_name}"), "rb") as f:
    ...
    # 训练流程
    if not is_grouped:
        train_epochs = 40 if dataset[receivers[0]].TF_df.shape[0] < 1000 else 60
    else:
        train_epochs = 80

       
    train_epochs = 60 if not is_grouped else 80
    ...
    train_model_with_ewc(model, all_loader, receivers, estimated_mean, estimated_fisher, 
                         epochs=train_epochs, lr=1e-3, device=device, lambda_ewc=1e2, save_fig_path=LOSS_DIR)
    # 保存模型
...
def main(adata_path):
    ...
    # 根据细胞数量划分receivers
    single_receivers, grouped_receivers = filter_receivers_by_cell_count(adata,min_cells=100, max_cells=500)
...
if __name__ == "__main__":
    adata_path = os.path.join(DATA_DIR, "4_human_melanoma_slide_tags/human_melanoma_processed.h5ad")
    main(adata_path)
